...
public class ElectrifiedLeverLogic : MonoBehaviour, Interactable
{
    ...
    public void onInteracted()
    {
        ...
        if (player.GetComponent<Player_Logic>().getStatus("Wet") != null)
        {
            Instantiate(electricShockParticlePrefab, player.transform.position, player.transform.rotation);
            player.GetComponent<Player_Logic>().addHp(-10, true);
        }
        else
        {
            if (isOn)
